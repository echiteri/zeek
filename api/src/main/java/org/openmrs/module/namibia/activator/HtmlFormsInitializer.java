...
import org.openmrs.module.namibia.utils.ExtensionFormUtil;
...
public class HtmlFormsInitializer implements Initializer {
	...
	protected static final String providerName = NamibiaConstants.MODULE_ID;
	...
	public synchronized void started() {
		log.info("Setting HFE forms for " + NamibiaConstants.MODULE_ID);
		...
		if (formsDir == null || formsDir.isDirectory() == false) {
			log.error("No HTML forms could be retrieved from the provided folder for " + providerName + ":" + formsPath);
			return;
		}
		log.info("Loading HTML forms from " + formsDir.getAbsolutePath());
